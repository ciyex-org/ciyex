...
//import org.ciyex.ehr.service.ChiefComplaintService;
...
public class ChiefComplaintController {
        ...
        try {
            ...
            if (items.isEmpty()) {
                return ResponseEntity.ok(ApiResponse.<List<ChiefComplaintDto>>builder()
                        .success(true)
                        .message("No Chief Complaints found for Patient ID: " + patientId)
                        .data(items)
                        .build());
            }
            return ResponseEntity.ok(ApiResponse.<List<ChiefComplaintDto>>builder()
            ...
            log.error("Error fetching Chief Complaints for Patient ID: " + patientId, ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<List<ChiefComplaintDto>>builder()
                            .success(false)
                            .message("Error fetching Chief Complaints for Patient ID: " + patientId + ". " + ex.getMessage())
                            .build());
        }
    }
    ...
    public ResponseEntity<ApiResponse<List<ChiefComplaintDto>>> list(
        ...
        try {
            ...
            if (items.isEmpty()) {
                return ResponseEntity.ok(ApiResponse.<List<ChiefComplaintDto>>builder()
                        .success(true)
                        .message(String.format("No Chief Complaints found for Patient ID: %d, Encounter ID: %d", patientId, encounterId))
                        .data(items)
                        .build());
            }
            return ResponseEntity.ok(ApiResponse.<List<ChiefComplaintDto>>builder()
            ...
            log.error("Error fetching Chief Complaints for Patient ID: " + patientId + ", Encounter ID: " + encounterId, ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<List<ChiefComplaintDto>>builder()
                            .success(false)
                            .message(String.format("Error fetching Chief Complaints for Patient ID: %d, Encounter ID: %d. %s", patientId, encounterId, ex.getMessage()))
                            .build());
        }
    }
    ...
    public ResponseEntity<ApiResponse<ChiefComplaintDto>> create(
        ...
        try {
            ...
            log.error("Validation error during Chief Complaint creation: " + ex.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(ApiResponse.<ChiefComplaintDto>builder().success(false).message(ex.getMessage()).build());
        } catch (IllegalStateException ex) {
            log.error("Business rule violation during Chief Complaint creation: " + ex.getMessage());
            return ResponseEntity.status(HttpStatus.LOCKED)
                    .body(ApiResponse.<ChiefComplaintDto>builder().success(false).message(ex.getMessage()).build());
        } catch (Exception ex) {
            log.error("Error creating Chief Complaint for Patient ID: " + patientId + ", Encounter ID: " + encounterId, ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<ChiefComplaintDto>builder()
                            .success(false)
                            .message("Error creating Chief Complaint: " + ex.getMessage())
                            .build());
        }
    }
    ...
    public ResponseEntity<ApiResponse<ChiefComplaintDto>> update(
        ...
        try {
            ...
            log.error("Validation error during Chief Complaint update: " + ex.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(ApiResponse.<ChiefComplaintDto>builder().success(false).message(ex.getMessage()).build());
        } catch (IllegalStateException ex) {
            log.error("Business rule violation during Chief Complaint update: " + ex.getMessage());
            return ResponseEntity.status(HttpStatus.LOCKED) // 423 LOCKED
                    .body(ApiResponse.<ChiefComplaintDto>builder().success(false).message(ex.getMessage()).build());
        } catch (Exception ex) {
            log.error("Error updating Chief Complaint for Patient ID: " + patientId + ", Encounter ID: " + encounterId + ", ID: " + id, ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<ChiefComplaintDto>builder()
                            .success(false)
                            .message("Error updating Chief Complaint: " + ex.getMessage())
                            .build());
        }
    }
    ...
    public ResponseEntity<?> print(
        ...
        try {
            ...
            log.error("Error printing Chief Complaint for Patient ID: " + patientId + ", Encounter ID: " + encounterId + ", ID: " + id, ex);
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(ApiResponse.<Void>builder().success(false).message(ex.getMessage()).build());
        } catch (Exception ex) {
            log.error("Error generating Chief Complaint PDF", ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .contentType(MediaType.APPLICATION_JSON)
